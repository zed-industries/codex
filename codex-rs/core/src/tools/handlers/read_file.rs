...
impl ToolHandler for ReadFileHandler {
    ...
    async fn handle(&self, invocation: ToolInvocation) -> Result<ToolOutput, FunctionCallError> {
        ...
        let collected = match mode {
            ReadMode::Slice => slice::read(&path, offset, limit, self.fs.as_ref()).await?,
            ReadMode::Indentation => {
                let indentation = indentation.unwrap_or_default();
                indentation::read_block(&path, offset, limit, indentation, self.fs.as_ref()).await?
            }
        };
...
mod slice {
    ...
    pub async fn read(
        ...
        let mut reader = fs.file_buffer(path, offset + limit).await.map_err(|err| {
...
mod indentation {
    ...
    pub async fn read_block(
        ...
        let collected = collect_file_lines(path, offset + limit, fs).await?;
    ...
    async fn collect_file_lines(
        ...
        let mut reader = fs.file_buffer(path, limit).await.map_err(|err| {
            FunctionCallError::RespondToModel(format!("failed to read file: {err}"))
        })?;
...
"
        ...
        let lines = read(temp.path(), 2, 2, &codex_apply_patch::StdFs).await?;
    ...
    async fn errors_when_offset_exceeds_length() -> anyhow::Result<()> {
        ...
        let err = read(temp.path(), 3, 1, &codex_apply_patch::StdFs)
            .await
    ...
    async fn reads_non_utf8_lines() -> anyhow::Result<()> {
        ...
        let lines = read(temp.path(), 1, 2, &codex_apply_patch::StdFs).await?;
...
"
    ...
    async fn truncates_lines_longer_than_max_length() -> anyhow::Result<()> {
        ...
        let lines = read(temp.path(), 1, 1, &codex_apply_patch::StdFs).await?;
...
"
        ...
        let lines = read_block(temp.path(), 3, 10, options, &codex_apply_patch::StdFs).await?;
...
"
        ...
        let expanded = read_block(temp.path(), 4, 50, options, &codex_apply_patch::StdFs).await?;
        assert_eq!(
            expanded,
            vec![
...
"
        ...
        let with_siblings =
            read_block(temp.path(), 3, 50, options, &codex_apply_patch::StdFs).await?;
        assert_eq!(
            with_siblings,
            vec![
                "L2:     if first {".to_string(),
...
"
        ...
        let lines = read_block(temp.path(), 1, 200, options, &codex_apply_patch::StdFs).await?;
        assert_eq!(
            lines,
            vec![
...
"
        ...
        let lines = read_block(temp.path(), 15, 200, options, &codex_apply_patch::StdFs).await?;
        assert_eq!(
            lines,
            vec![
                "L10:         init() {".to_string(),
...
"
    ...
    async fn indentation_mode_handles_cpp_sample_shallow() -> anyhow::Result<()> {
        ...
        let lines = read_block(temp.path(), 18, 200, options, &codex_apply_patch::StdFs).await?;
        assert_eq!(
            lines,
            vec![
